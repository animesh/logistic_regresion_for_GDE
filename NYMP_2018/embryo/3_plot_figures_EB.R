#################### Reproduction and modification of Supplementary Figure 6 of the Ntranos, Yi,
#################### Melsted and Pachter 2019 paper.  This script is adapted from
#################### https://github.com/pachterlab/NYMP_2018/blob/master/embryo/3_plot_figures.R
#################### It's most important modification is to subset on commonly-assayed genes before
#################### computing sizes of intersection Modifications from the original script are
#################### listed using ## EB and ## /EB flags You should run the rest (scripts 0, 1 and
#################### 2) of the https://github.com/pachterlab/NYMP_2018/tree/master/embryo directory
...
## EB: Making sure we use the same format for gene IDs (some are
## ensembl_ids.version and some are only ensembl_ids)
...
## ##EB ## Number of 0s per tx per condition w=grepl('E4',colnames(tx_counts))
## E4_0=(tx_counts[,w]==0) E3_0=(tx_counts[,!w]==0)

## E4_ag=aggregate_df(E4_0,t2g[match(rownames(E4_0),t2g[,2]),1],sum)
## E3_ag=aggregate_df(E3_0,t2g[match(rownames(E3_0),t2g[,2]),1],sum)

## E4_ag=rowSums(E4_ag) E3_ag=rowSums(E3_ag)

## wmast=rownames(subset(l[[5]][shared,],p_val<0.05))
## wlr=rownames(subset(l[[1]][shared,],pvalues<0.05))

## png(width=2000,height=2000,res=300,'counting_0s.png') par(mfrow=c(2,2))
## freqplot(E4_ag[intersect(wmast,wlr)],E3_ag[intersect(wmast,wlr)]);title(main='D.E.
## for both LR & MAST');abline(a=0,b=sum(!w)/sum(w));title(xlab='Sum of 0 in tx
## counts in E4 cells [ag to gene]',ylab='Sum of 0 in tx counts in E3 cells [ag to
## gene]')
## freqplot(E4_ag[setdiff(wmast,wlr)],E3_ag[setdiff(wmast,wlr)]);title(main='Unique
## to MAST');abline(a=0,b=sum(!w)/sum(w))
## freqplot(E4_ag[setdiff(wlr,wmast)],E3_ag[setdiff(wlr,wmast)]);title(main='Unique
## to LR');abline(a=0,b=sum(!w)/sum(w))
## freqplot(E4_ag[setdiff(shared,c(wmast,wlr))],E3_ag[setdiff(shared,c(wmast,wlr))]);title(main='Negative
## for both');abline(a=0,b=sum(!w)/sum(w)) dev.off()
