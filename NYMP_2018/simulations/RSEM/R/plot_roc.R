...
de_dir <- file.path(base_dir, "de")  ## I guess that these are the results of the various DE frameworks on the simulated data ?
adjust <- FALSE

t2g <- read.table("~/transcriptomes/Homo_sapiens.GRCh38.rel79.transcripts", stringsAsFactors = FALSE, 
    sep = "\t")
colnames(t2g) <- c("transcripts", "genes")  ## Transcript to genes mapping, can extract from the 10x python notebook
...
table <- merge(deseq, mast, by = "genes", all = TRUE)
table <- merge(table, scde, by = "genes", all = TRUE)
table <- merge(table, LR, by = "genes", all = TRUE)
table <- merge(lan, table, by = "genes", all = TRUE)
table <- merge(LR_tx, table, by = "genes", all = TRUE)
table <- merge(tobit, table, by = "genes", all = TRUE)
table <- merge(table, LR_deseq, by = "genes", all = TRUE)
table <- merge(table, LR_tpms, by = "genes", all = TRUE)
table <- merge(table, LR_univariate, by = "genes", all = TRUE)

table <- select(table, genes, mast, deseq, scde, lan, LR, LR_tx, n_tccs, n_transcripts, 
    tobit, LR_deseq, LR_tpms, LR_univariate)
n <- 52
...
mast <- calculate_fdr(table$perturb, table$mast, "mast")
deseq <- calculate_fdr(table$perturb, table$deseq, "deseq")
scde <- calculate_fdr(table$perturb, table$scde, "scde")
tobit <- calculate_fdr(table$perturb, table$tobit, "tobit")
...
names <- c("DESeq2", "Monocle - Tobit", "MAST", "SCDE", "log reg - transcripts", 
    "log reg - gene counts", "log reg - TCCs")
...
p <- ggplot(data = fdrs, aes(x = FDR, y = sensitivity, colour = Method)) + geom_path() + 
    scale_colour_manual(values = c("#F8766D", "#A3A500", "#00BF7D", "#00B0F6", "blue", 
        "purple", "purple4"))
p <- p + theme_gray(15)
...
names <- c("DESeq2", "Monocle - Tobit", "MAST", "SCDE", "log reg - raw counts", "log reg - TPM Norm", 
    "log reg - DESeq2 Norm")

fdrs <- lapply(1:length(fdrs), function(x) data.frame(sensitivity = fdrs[[x]]$sen, 
    specificity = fdrs[[x]]$specificity, Method = names[[x]]))

fdrs <- do.call(rbind, fdrs)

p <- ggplot(data = fdrs, aes(x = FDR, y = sensitivity, colour = Method)) + geom_path() + 
    scale_colour_manual(values = c("grey", "grey", "grey", "grey", "darkcyan", "blueviolet", 
        "blue"))
