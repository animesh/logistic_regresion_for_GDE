...
# args <- commandArgs(TRUE) base_dir <- args[[1]]
...
runSCDE <- function(counts, sg) {
    error_models <- scde.error.models(counts = counts, groups = sg, n.cores = 20, 
    ...
    scde_DE <- scde.expression.difference(error_models, counts, prior, sg, verbose = 1, 
        n.cores = 20, n.randomizations = 100)
