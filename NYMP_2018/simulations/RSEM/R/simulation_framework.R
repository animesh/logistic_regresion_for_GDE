...
## need to read without normalizing differently, or maybe by normalizing using
## DEseq
...
## can perturb genes with some zero transcripts - only nonzero transcripts get
## perturbed
choose_genes_to_perturb <- function(fits, fraction) {
    ...
    ## only perturb genes that are nonzero transcript to gene table
    ...
    t2g_perturb <- merge(t2g_perturb, perturb_gene_table, by = c("perturbed_genes"), 
        left.all = TRUE)
...
## to change between log and nonlog model, exp versus do not exp the model. but
## both equate to lognormal effects with at least 2 drawing effects size from
## normal
draw_effects_size <- function(nsample) {
    ## DESeq drew from log normal with mean =0 rlnorm(nsample, meanlog=0, sdlog=1.5) I
    ## will do the same but now with truncated log normal change to normal because in
    ## log counts
...
## to change between log and nonlog model, change effectsize*mean versus
## effectsize + mean. do not need to retransform
...
map_sims <- function(sims) {
    ...
    tpms <- lapply(1:ncells, function(i) {
        sapply(sims, function(x) x[i])
    })
    tpms
}
...
print_tables <- function(template_table, tpms, folder, type = "tpms") {
    ...
    for (i in 1:ncells) {
        ...
        write.table(template_table, file = title, quote = FALSE, row.names = FALSE, 
            sep = "\t")
    }
}
