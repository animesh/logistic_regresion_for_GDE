...
fdrs <- lapply(1:length(fdrs), function(x) data.frame(sensitivity = fdrs[[x]]$sensitivity, 
    FDR = fdrs[[x]]$fdr, Method = names[[x]]))
fdrs <- do.call(rbind, fdrs)

library(ggplot2)
tiff("./fdr.tiff", height = 3000, width = 4000, res = 500)
g <- ggplot(fdrs, aes(FDR, sensitivity, colour = Method)) + geom_path() + scale_colour_manual(values = c("#F8766D", 
    "#A3A500", "#00BF7D", "#00B0F6", "blue", "purple", "purple4")) + theme_grey(15)
print(g)
dev.off()
...
rocs <- lapply(1:length(rocs), function(x) data.frame(sensitivity = rocs[[x]]$sensitivities, 
    specificity = rocs[[x]]$specificities, Method = names[[x]]))
rocs <- do.call(rbind, rocs)

tiff("./roc.tiff", height = 3000, width = 4000, res = 500)
g <- ggplot(rocs, aes(1 - specificity, sensitivity, colour = Method)) + geom_path() + 
    scale_colour_manual(values = c("#F8766D", "#A3A500", "#00BF7D", "#00B0F6", "blue", 
        "purple", "purple4")) + theme_grey(15)
print(g)
dev.off()
