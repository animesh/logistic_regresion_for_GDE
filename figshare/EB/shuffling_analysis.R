...
## I have had issues running this script in parallel (memory issues, forking crashes) so I advise against mc.cores>1.
## scipen=999 is used to avoid exponential notation when converting numerics with as.character()
options("mc.cores" = 1,
        scipen = 999)
...
## 10X data (Zheng et al, Nat Com, 2017). Processed by Ntranos et al.
...
TX_TO_ENST <- read.csv("../../NYMP_2018/10x_example-logR/TX_to_ENST.csv",
                       sep = "\t", stringsAsFactors = FALSE, header = FALSE)
TX_TO_ENST <- setNames(TX_TO_ENST[, 2], TX_TO_ENST[, 1])

TX_TO_ENSG <- read.csv("../../NYMP_2018/10x_example-logR/TX_to_ENSG.csv",
                       sep = "\t", stringsAsFactors = FALSE, header = FALSE)
TX_TO_ENSG <- setNames(TX_TO_ENSG[, 2], TX_TO_ENSG[, 1])

ENSG_to_name <- read.csv("../../NYMP_2018/10x_example-logR/ENSG_to_name.csv",
                         sep = "\t", stringsAsFactors = FALSE, header = FALSE)
ENSG_to_name <- setNames(ENSG_to_name[, 2], ENSG_to_name[, 1])

EC_dict <- read.csv("../../NYMP_2018/10x_example-logR/matrix.ec",
                    sep = "\t", stringsAsFactors = FALSE, header = FALSE)
...
## Load public dataset EMTAB3929 (Petropoulos et al)
## http://imlspenticton.uzh.ch/robinson_lab/conquer/data-mae/EMTAB3929.rds
## Here we compare the samples E3 vs E4, and thusly subset the data
EMTAB3929 <- readRDS("../../data/EMTAB3929.rds") 
EMTAB3929 <- EMTAB3929[, grepl("E3", colnames(EMTAB3929)[["tx"]]) | grepl("E4", colnames(EMTAB3929)$tx)] 

## Load public dataset GSE64016
## http://imlspenticton.uzh.ch/robinson_lab/conquer/data-mae/GSE64016.rds
## Here we compare cells at the G1 vs S phases of the cell cycle, and thusly subset the data
GSE64016 <- readRDS("../../data/GSE64016.rds")
g1s <- c("single H1-Fucci cell sorted from G1 phase of the cell cycle only",
         "single H1-Fucci cell sorted from S phase of the cell cycle only")
GSE64016 <- GSE64016[, colData(GSE64016)$source_name_ch1 %in% g1s] 

## List datasets and harmonize. List of list.
## Sublists elements are:
##   1) xp : expression matrix,
##   2) grps : vector of cell types,
##   3) name : dataset's name,
##   4) t2g: Transcript (or TCCs) to gene mapping
data <- list(
  list(
  ...
  ## Groups = G1 vs S phase of the cell cycle
  list(
    xp = assays(GSE64016)$tx,
    grps = colData(GSE64016)$source_name_ch1 == "single H1-Fucci cell sorted from G1 phase of the cell cycle only",
    name = "GSE64016",
    t2g = rowData(GSE64016[["tx"]])$gene
  )
)

## Choose threshold for inclusion of transcripts in the
## analysis (expressed in what fraction of the cells)
cutoffs_expressed <- c(0.001, 0.2)

## Compute TPMs, expression frequency of transcripts,
## binarized expression frequency, cellular detection rate
data <- lapply(
    data,
    function(x) {
        print(x$name)
        ...
        if (is(xp, "sparseMatrix")) {
            ...
            xp_tpm <- xp %*% Diagonal(x = 1 / Matrix:::colSums(xp)) * 10^6 
            xp_logtpm <- xp_tpm
            xp_logtpm@x <- log2(1 + xp_logtpm@x)
        } else {
            ## Compute TPMs        
            xp_log <- log2(1 + xp)
            xp_tpm <- xp %*% diag(1 / colSums(xp)) * 10^6 
            xp_logtpm <- log2(1 + xp_tpm)
        }
        ## Per transcript, in what fraction of the cells in the transcript expressed
        expression_frequency <- rowSums(as.matrix(xp) > 0) / ncol(xp) 
        sca <- SingleCellExperiment(
            assays = list(
                counts = as.matrix(xp),
                logcounts = as.matrix(xp_log),
                tpm = as.matrix(xp_tpm),
                logtpm = as.matrix(xp_logtpm)),
            colData = data.frame(
                Group = x$grps, ## Cell type labels
                ## Cellular Detection Rate (See Finak et al, Genome Biology, 2015)
                CDR = colSums(as.matrix(xp) > 0) 
            ),
            ...
            rowData = data.frame(
                ...
                ## For each threshold, boolean indicating whether the transcript
                ## passes the threshold or not                
                sapply(cutoffs_expressed, function(co) {
                    expression_frequency > co
                }), 
                stringsAsFactors = FALSE
            )
        )
        nc <- ncol(rowData(sca))
        ss <- seq(nc - length(cutoffs_expressed) + 1, nc, by = 1)
        colnames(rowData(sca))[ss] <- make.names(paste0("binary_", cutoffs_expressed))
        x <- c(x, list(sca = sca))
        return(x)
    }
)
...
## Cleanup workspace
rm(list = setdiff(ls(), c("data", "do_logreg", "do_mast_gn", "do_mast_tx", "do_t_tests", "sidak", "splatter_dir", "cutoffs_expressed", "split_matrix", "aggregate_df", "makeTransparent")))
gc()
...
## use subsampled data (code below) to test the routine
## =============================================================================

## -----------------------------------------------------------------------------
## ## Testing code: subset down to small number of rows per dataset
## ## Set aside the full dataset
## data_full <- data 

## ## Subsample to 1000 feature observations
## data_subsample <- lapply(data_full, function(x, subsample = 1000) {
##     spl <- sample(1:nrow(x$xp), subsample)
##     x$xp <- x$xp[spl, ]
##     x$t2g <- x$t2g[spl]
##     x$sca <- x$sca[spl, ]
##     x
...
## ## Save the full dataset aside, use subsample as main data
## data <- data_subsample
## -----------------------------------------------------------------------------
        ...
        function(x) {
            ...
            L <- lapply(
                ...
                function(co) {
                    ...
                    ## split matrix into rows mapping to the same gene          
                    xp_split <- split_matrix(assays(sca)$counts, as.character(rowData(sca)$gene_ensembl)) 
                    xp_split <- lapply(xp_split, t)
                    ## Make sure that your column names are compatible with R formulas
                    xp_split <- lapply(xp_split, function(y) {
                        colnames(y) <- make.names(colnames(y)) 
                        y
                    })
                    ## Append CDR and cell types as new columns
                    xp_split <- lapply(xp_split, cbind, grps = colData(sca)$Group, CDR = colData(sca)$CDR) 
                    xp_split <- lapply(xp_split, as.data.frame)
                    
                    lr_CDR_noscrbl <- unlist(mclapply(
                        xp_split,
                        function(y) {
                            fmla <- paste0("grps~", paste0(setdiff(colnames(y), c("grps")), collapse = "+"))
                            lrtest(
                                glm(formula = fmla,
                                    data = y,
                                    family = binomial()),
                    ...
                    lr_noCDR_noscrbl <- unlist(mclapply(
                        xp_split,
                        function(y) {
                            ## We remove CDR from the formula since we do not want to include it
                    ...
                    return_names <- apply(
                        expand.grid(c("mast", "lr"), c("CDR", "noCDR"), c("noscrbl", "scrbl")),
                        1, paste0, collapse = "_")
...
## =============================================================================
## 3. Perform GDE on cells from the same batch (randomly affected to two distinct subgroups)
## =============================================================================
                    ...
                    sapply(
                        ...
                        function(celltype) {
                            ...
                            lr_CDR_scrbl <- unlist(mclapply(
                                ...
                                function(y) {
                                    fmla <- paste0("grps~",
                                                   paste0(setdiff(colnames(y), c("grps")),
                                                          collapse = "+"))
                                    lrtest(
                                        glm(formula = fmla,
                                            data = y,
                            ...
                            lr_noCDR_scrbl <- unlist(mclapply(
                                ...
                                function(y) {
                                    fmla <- paste0("grps~",
                                                   paste0(setdiff(colnames(y), c("grps", "CDR")),
                                                          collapse = "+"))
                                    lrtest(
                                        glm(formula = fmla,
                                            data = y,
                            ...
                            return_names <- apply(
                                expand.grid(c("mast", "lr"), c("CDR", "noCDR"), c("scrbl")),
                                1, paste0, collapse = "_")

                            return(
                                list(
                                    pvalues = mget(return_names),
                                    scrbl = colData(sca)[, "Group_shuffled", drop = FALSE]
                                )
                            )
                        },
                        simplify = FALSE
...
methods <- apply(expand.grid(c("mast", "lr"), c("CDR", "noCDR"), c("noscrbl", "scrbl")), 1, paste0, collapse = "_")
colors <- setNames(rep(c("red", "blue"), 4), methods)
linetype <- setNames(rep(c("solid", "dotted", "dashed", "dotdash"), each = 2), methods)
...
for (cutoff in as.character(cutoffs_expressed)) {
    ...
    ps <- lapply(names(data), function(ds) {
        ...
        df <- data.table(
            lr_noCDR_noscrbl = data[[ds]]$degs[[cutoff]]$lr_noCDR_noscrbl$gn,
            lr_CDR_noscrbl = data[[ds]]$degs[[cutoff]]$lr_CDR_noscrbl$gn,
            lr_noCDR_scrbl = data[[ds]]$degs[[cutoff]]$lr_noCDR_scrbl$gn,
...
ps <- lapply(names(data), function(ds) {
    ...
    p1 <- ggplot(data = subset(df, grepl("mast", df$alg)),
                 aes(x = p, y = n, group = group, colour = freqs_bin, linetype = method)) +
        geom_path(size = 1) +
        scale_colour_manual(values = colors) +
        scale_linetype_manual(values = linetype) +
        xlab("p") +
    ...
    p2 <- ggplot(data = subset(df, grepl("lr", df$alg)),
                 aes(x = p, y = n, group = group, colour = freqs_bin, linetype = method)) +
        geom_path(size = 1) +
        scale_colour_manual(values = colors) +
        scale_linetype_manual(values = linetype) +
    ...
    p3 <- ggplot(data = subset(df, grepl("mast", df$alg)),
                 aes(x = q, y = n, group = group, colour = freqs_bin, linetype = method)) +
        geom_vline(xintercept = 0.05, linetype = "dotted") +
        geom_path(size = 1) +
        scale_colour_manual(values = colors) +
        scale_linetype_manual(values = linetype) +
    ...
    p4 <- ggplot(data = subset(df, grepl("lr", df$alg)),
                 aes(x = q, y = n, group = group, colour = freqs_bin, linetype = method)) +
        geom_vline(xintercept = 0.05, linetype = "dotted") +
        geom_path(size = 1) +
        scale_colour_manual(values = colors) +
    ...
    for (co in as.character(cutoffs_expressed)) {
        for (celltype in names(data[[ds]]$negs[[co]])) {
            for (feature_type in c("tx", "gn")) {
                dt <- as.data.table(sapply(data[[ds]]$negs[[co]][[celltype]]$pvalues, function(x) {
                    x[[feature_type]]
                }))
