## =============================================================================
## README:
## This script does splatter simulations based either on default splatter
## parameters or parameters used by Nnatros et al, with modifications:
##
...
##   4. We downsample simulated datasets if relevant (to study the impact of dataset
##      sizes, degree of confusion by batches...)
##   5. Then gene differential expression using MAST and multivariate LR is ran.
##      The results are exported to disk
##   6. Plot the results
## =============================================================================
...
## Downloaded from:
## https://figshare.com/articles/Single_Cell_RNA-Seq_Simulations_for_Ntranos_Yi_et_al_2019/7108364
...
## The splat_authors object is generated by splatter then subsetted so that cell
## populations (colData(splat_authors)$Group) are balanced (105 vs 105 cells). The
## following makes sure that the subsetting is done as per Nnatros et al.
...
## Extract splatter parameters as per Ntranos et al to
## perform further simulations with the same template
params <- splat_authors@metadata$params

## Fix missing parameters from the template
## (likely due to splatter update at some point)
attributes(params)$lib.norm <- FALSE
attributes(params)$dropout.type <- "none"
attributes(params)$path.nSteps <- attributes(params)$path.length
...
if (file.exists(file.path(splatter_dir, "t2g.rds"))) {
    ...
    t2g <- getBM(attributes = c("ensembl_gene_id", "ensembl_transcript_id"), values = rownames(splat_authors), 
        filters = "ensembl_transcript_id", mart = mart)
...
simulations <- list(
    list(
        name = "vanilla",
        params = newSplatParams(nGenes = length(rows), 
                                batchCells = 300,
...
names(simulations) <- sapply(simulations, function(x) {
    x$name
})
...
simulations <- c(list(authors = list(name = "authors", params = params, splat = splat_authors, 
    downsampling_matrix = matrix(nrow = 1, ncol = 2, dimnames = list("Batch1", paste0("Group", 
        1:2)), data = c(105, 105)), downsampling_indices = which(paste0("Cell", 1:300) %in% 
        colnames(splat_authors)))), simulations)

## Computing tpms, cntxon...
simulations <- lapply(simulations, function(x) {
    splat <- x$splat

    ## cntxon = Cellular Detection Rate (CDR) at the transcript level (see Finak et al, Genome Biology, 2015)
    colData(splat)$cntxon <- colSums(assays(splat)$counts > 0)

    ## Necessary for the do_mast_tx wrapper
    rowData(splat)$transcript <- rownames(rowData(splat))  

    ## Necessary for the do_mast_tx wrapper
    rowData(splat)$gene_ensembl <- splatter_t2g[rownames(rowData(splat))]  

    ## Computing TPMs
    assays(splat)$tpm <- assays(splat)$count %*% diag(1/colSums(assays(splat)$count)) * 10^6 
    x$splat <- splat
    x
})
...
## Contingency tables for downsampling. Columns = Groups (cell populations) and
## Rows = Batches. Names of 1st level = current names, Names of 2nd level = name
## of downsampled element
downsampling <- list(
    authors = list(
...
## Actual downsampling
simulations <- lapply(names(downsampling), function(x) {
    lapply(names(downsampling[[x]]), function(y) {
        set.seed(234)
        dn_mat <- downsampling[[x]][[y]]
        dimnames(dn_mat) <- list(paste0("Batch", 1:nrow(dn_mat)), paste0("Group", 
            1:ncol(dn_mat)))
        ...
        indices <- lapply(combs, function(z) {
            sample(which(colData$Batch == z[1] & colData$Group == z[2]), size = dn_mat[z[1], 
                z[2]])
        })
...
## Add gene-aggregated data to simulations
simulations <- lapply(simulations, function(x) {
    ## Aggregate transcripts to gene counts by summation
    count <- aggregate_df(assays(x$splat)$count,
                          rowData(x$splat)$gene_ensembl,
                          fun = sum, 
                          margin = 1)  
    cData <- colData(x$splat)
    fData <- rowData(x$splat)

    ## rowData only keeps track of whether the gene is differentially expressed
    ## defined as true if any of its constituent transcripts are differentially expressed
    fData <- aggregate_df(as.matrix(fData)[, grepl("DEFacGroup", colnames(fData))], 
        fData$gene_ensembl, fun = function(y) {
    ...
    ## Necessary for the do_mast_tx wrapper
    fData <- cbind(fData, gene_ensembl = rownames(fData), transcript = rownames(fData))

    ## Create the SingleCellAssay object
    splat_gn <- FromMatrix(count, cData = cData, fData = as(fData, "DataFrame"), 
        check_sanity = FALSE)  
    names(assays(splat_gn)) <- "counts"

    ## cntxon = Cellular Detection Rate (CDR) at the transcript level (see Finak et al, Genome Biology, 2015)
    colData(splat_gn)$cntxon <- colSums(assays(splat_gn)$counts > 0)  
    rowData(splat_gn)$transcript <- rownames(rowData(splat_gn))
    rowData(splat_gn)$gene_ensembl <- rownames(rowData(splat_gn))

    ## compute TPMs
    assays(splat_gn)$tpm <- assays(splat_gn)$count %*% diag(1/colSums(assays(splat_gn)$count)) * 10^6
...
write.table(
    do.call(rbind, sapply(simulations, function(x) {
        data.frame(
            name = x$name,
    ...
    file = "../splatter/tables/simulations_parameters.csv", sep = ",", 
    row.names = FALSE
)
...
## =============================================================================
## 5. Perform Differential Expression Gene Analysis
## =============================================================================
...
simulations <- lapply(simulations, function(x) {
    ...
    if (!file.exists(file)) {
        ...
        ## MAST
        fmla <- if (length(unique(colData(x$splat)$Batch)) > 1) {
            "~Group+Batch"
        } else {
            "~Group"
        }

        ## Model is (transcript counts ~ Group + Batch) if we simulate Batch,
        ## otherwise it is just (transcript ocunts ~ Group). We do not use cntxon
        ## as it is not informative in splatter so there is no reason to include
        ## it in the model

        ## Generates MAST on transcripts + MAST-Sidak on genes
        mast_tpm <- do_mast_tx(
            sca = FromMatrix(log2(1 + assays(x$splat)$tpm),
                             cData = colData(x$splat), 
                             fData = rowData(x$splat)),
            fmla = fmla,
            LRT = "GroupGroup2")

        ## Generates MAST on gene counts
        mast_unaggregated_gn_tpm <- do_mast_tx(
            sca = FromMatrix(log2(1 + assays(x$splat_gn)$tpm), 
                             cData = colData(x$splat_gn), fData = rowData(x$splat_gn)),
            fmla = fmla, 
            LRT = "GroupGroup2")  
        ...
        ## mLR on transcripts
        lr_tpm <- do_logreg(
            data = assays(x$splat)$tpm,
            response = colData(x$splat)$Group == "Group2",
            tx2gene = rowData(x$splat)$gene_ensembl, covariates = covariates
        )
        
        ## univariate LR on transcripts
        lr_unaggregated_tpm <- do_logreg(
            data = assays(x$splat)$tpm,
            response = colData(x$splat)$Group == "Group2",
            tx2gene = rowData(x$splat)$transcript,
            covariates = covariates
        )

        ## univariate LR on gene counts
        lr_unaggregated_gn_tpm <- do_logreg(
            data = assays(x$splat_gn)$tpm,
            response = colData(x$splat_gn)$Group == "Group2",
            tx2gene = rowData(x$splat_gn)$transcript,
            covariates = covariates)  
        
        ## ANOVA
        aov_tpm <- do_t_tests(
            response = colData(x$splat)$Group, data = log2(1 + assays(x$splat)$tpm),
            t2g = rowData(x$splat)$gene_ensembl,
            covariates = covariates_aov)
        
        aov_unaggregated_gn_tpm <- do_t_tests(
            response = colData(x$splat_gn)$Group, 
            data = log2(1 + assays(x$splat_gn)$tpm),
            t2g = rowData(x$splat_gn)$gene_ensembl, 
            covariates = covariates_aov)
...
lapply(simulations, function(x) {
    ## Compute which genes are differentially expressed
    ## add into rowData(x$splat)
    print(x$name)

    de_transcripts <- rowSums(
        as.matrix(rowData(x$splat)[, grepl("DEFacGroup", colnames(rowData(x$splat)))]) != 1
    ) > 0
    ...
    p <- ggplot(data = df, aes(x = values)) + geom_histogram(bins = 20, aes(y = ..density..), 
        fill = "gray") + theme_bw() + xlim(0, 1) + facet_wrap(~ind, scales = "free")
    print(p)
    dev.off()
    
    ## Generate 5-panel figures with 1) empirical FDR vs TPR [as per Nnatros et al],
    ## 2) FPR vs TPR [ROC curve], 3) nominal FDR (p-values from each GDE method +
    ## p.adjust(.,method='fdr') versus empirical FDR, 4) nominal FDR vs FPR and 5)
    ## nominal FDR vs sensitivity
    file <- file.path(splatter_dir, "graphs", "roc_curves", paste0(x$name, ".png"))
    png(file, width = 6000, height = 3000, res = 300, type = 'cairo-png')
    ...
    fdrs <- sapply(names(x$degs_formatted), function(y) {
        ...
        calculate_fdr(true_DE = true_DE[names(x$degs_formatted[[y]])], pvalues = x$degs_formatted[[y]], 
            title = y)
    }, simplify = FALSE)
    
    fdrs <- lapply(1:length(fdrs), function(y) {
        data.frame(sensitivity = fdrs[[y]]$sen, 
    ...
    colors <- setNames(makeTransparent(rep(brewer.pal(3, "Set1"), each = 3), 123), 
                       unique(fdrs$Method))
    linetype <- setNames(rep(c("solid", "dashed", "dotted"), 3), unique(fdrs$Method))

    p1 <- ggplot(data = fdrs, aes(x = actual_FDR, y = sensitivity, colour = Method, 
        linetype = Method)) + scale_colour_manual(values = colors) + scale_linetype_manual(values = linetype) + 
        geom_path() + theme_bw(12)
    p2 <- ggplot(data = fdrs, aes(x = FalsePositiveRate, y = sensitivity, colour = Method, 
        linetype = Method)) + scale_colour_manual(values = colors) + scale_linetype_manual(values = linetype) + 
        geom_path() + geom_abline(intercept = 0, slope = 1, linetype = "dashed") + 
        theme_bw(12)
    p3 <- ggplot(data = fdrs, aes(x = nominal_FDR, y = actual_FDR, colour = Method, 
        linetype = Method)) + scale_colour_manual(values = colors) + scale_linetype_manual(values = linetype) + 
        geom_path() + geom_vline(xintercept = 0.05, linetype = "dashed") + geom_abline(intercept = 0, 
        slope = 1, linetype = "dotted") + theme_bw(12)
    p4 <- ggplot(data = fdrs, aes(x = nominal_FDR, y = FalsePositiveRate, colour = Method, 
        linetype = Method)) + scale_colour_manual(values = colors) + scale_linetype_manual(values = linetype) + 
        geom_path() + geom_vline(xintercept = 0.05, linetype = "dashed") + theme_bw(12)
    p5 <- ggplot(data = fdrs, aes(x = nominal_FDR, y = sensitivity, colour = Method, 
        linetype = Method)) + scale_colour_manual(values = colors) + scale_linetype_manual(values = linetype) + 
        geom_path() + geom_vline(xintercept = 0.05, linetype = "dashed") + theme_bw(12)
...
all_sim <- sapply(simulations, function(x) {
    ...
    ## For each possible p-value threshold, tabulate TPR, FPR, fraction of genes
    ## considered GDE
...
## While we simulate with dropout being differential across cell subgroups,
## it does not make sense to compute the statistics we compute in this case.
## Indeed, differential dropout is computed for each gene independently of
## whether the gene is differentially expressed or not, which is arguably wrong.
all_sim <- subset(all_sim, !grepl("dropout", all_sim$simulation))  

## Plot nominal FDR vs Sensitivity and nominal FDR vs FPR (for all simulations, in
## distinct panels)
...
p1 <- ggplot(data = all_sim, aes(x = q, y = sens, group = method, colour = method)) + 
    geom_path(size = 1) + scale_colour_manual(values = colors) + scale_linetype_manual(values = linetype) + 
    xlab("Q") + ylab("Sensitivity") + theme_bw() + ggtitle("Sensitivity") + xlim(0, 
    1) + facet_wrap(~simulation)

p2 <- ggplot(data = all_sim, aes(x = q, y = FPR, group = method, colour = method, 
    linetype = method)) + geom_path(size = 1) + scale_colour_manual(values = colors) + 
    scale_linetype_manual(values = linetype) + xlab("Q") + ylab("False Positive Rate") + 
    theme_bw() + ggtitle("False Positive Rate") + xlim(0, 1) + facet_wrap(~simulation)

file <- file.path(splatter_dir, "graphs", "paper_roc_curves_all_faceted.png")
png(file, width = 6000, height = 1000, res = 300)
p1 + p2
dev.off()
...
all_sim_interp <- lapply(all_sim_interp, function(x) {
    ...
    res[, sens := approx(x = x$p, y = x$sens, xout = interp_points)$y]
    ## Linear interpolation on a common set of points for future pairwise plotting
    res[, FPR := approx(x = x$p, y = x$FPR, xout = interp_points)$y]
    res[p == 0, c("sens", "FPR") := 0]
    res[p == 1, c("sens", "FPR") := 1]
...
p1 <- ggplot(data = all_sim_interp, aes(x = p, y = sens, colour = simulation)) + 
    geom_path(size = 1) + xlab("P") + ylab("Sensitivity_mLR - Sensitivity_MAST") + 
    theme_bw() + xlim(0, 1) + ## facet_wrap(~simulation) +
ylim(-0.2, ytop) + geom_hline(yintercept = 0, linetype = "dashed") + ggtitle("Sensitivity")

p2 <- ggplot(data = all_sim_interp, aes(x = p, y = FPR, colour = simulation)) + geom_path(size = 1) + 
    xlab("P") + ylab("FPR_mLR - FPR_MAST") + theme_bw() + ylim(-0.2, ytop) + ## facet_wrap(~simulation) +
xlim(-0, 1) + geom_hline(yintercept = 0, linetype = "dashed") + ggtitle("FPR")
